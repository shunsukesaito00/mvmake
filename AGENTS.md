...
| `e2e/` | Playwright E2E テスト（`npm run test:e2e` 全量 1075 件 = `basic` 811 + `editor` 264。本番スモーク `test:e2e:prod` は **basic 811 のみ**） |
...
| `docs/PROD_SMOKE_AUDIT.md` | 本番スモーク E2E（`test:e2e:prod`・811 シナリオ）（v3.24.0 / Phase H G32・Phase F F1 含） |
