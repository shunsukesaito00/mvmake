...
| `e2e/` | Playwright E2E テスト（`npm run test:e2e` 全量 1072 件 = `basic` 810 + `editor` 262。本番スモーク `test:e2e:prod` は **basic 810 のみ**） |
...
| `docs/PROD_SMOKE_AUDIT.md` | 本番スモーク E2E（`test:e2e:prod`・810 シナリオ）（v3.23.0 / Phase H G31・Phase F F1 含） |
