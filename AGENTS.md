...
| `e2e/` | Playwright E2E テスト（`npm run test:e2e` 全量 1069 件 = `basic` 809 + `editor` 260。本番スモーク `test:e2e:prod` は **basic 809 のみ**） |
...
| `docs/PROD_SMOKE_AUDIT.md` | 本番スモーク E2E（`test:e2e:prod`・809 シナリオ）（v3.22.0 / Phase H G30・Phase F F1 含） |
