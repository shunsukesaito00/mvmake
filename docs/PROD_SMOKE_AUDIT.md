...
※ v2.5.29 で user→builtin 組み込みルック2回再クリック挟みは plain→suffix リネーム＋末尾 plain 3件追加済み（471→474）。v2.5.41 では既存 plain 3件（行13412/13474/13536付近）に `・builtin-double-named-reclick` 接尾辞を付与し、重複していた旧 suffix 3件を削除、接尾辞なしタイトル3件を末尾に再配置（`user-then-builtin-bundled-lut-intensity-*-builtin-double-named-reclick-*` ファイル名）。件数は 507 維持。
