...
- **ローリング編集**（選択ツール時、隣接クリップ境界の編集点ハンドルをドラッグして前後同時トリム）
- **キーフレーム前後ジャンプ**（`;` / `'` で変形・音量・速度キーフレームを統合ナビ）
- **動画音声リンク**（内蔵音声の切り離し/リンク・ナレーション配置準備・BGMダッキング連動）
- **色調一括ペースト**（ルック・LUT・カーブのコピー/ペースト・マルチ選択へ一括適用）
...
- J/K/L 再生ショートカット（J=1秒戻る / K=停止 / L=再生・L連打/長押しで 2x/4x 順再生）
...
npm run test:e2e  # E2Eテスト (Playwright、初回は npx playwright install chromium が必要)。全量 1066 件 = basic 808 + editor 258
npm run test:e2e:prod # 公開環境(GitHub Pages)に対するスモークE2E（basic 808 のみ。editor 258 はローカル/CI 全量）
...
|------|------|
| Space | 再生/停止 |
| J / K / L | 1秒戻る / 停止 / 再生（Shift+J: 1フレーム戻る・L連打/長押しで 2x/4x） |
| ← / → | フレーム送り |
| I / O | In点 / Out点 |
| M | 章マーカー追加 |
| Shift + M | ビートマーカー追加 |
