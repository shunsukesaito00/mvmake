...
- 色調補正（明るさ・コントラスト・彩度・トーンカーブ・RGB カーブ・HSL）・3D LUT（.cube インポート、Canvas ミニプレビュー、ルックプリセットと併用可）
- クロップ、Ken Burns（ズームパン）
- BGMフェードイン/アウト、BGMダッキング（動画音声区間で自動減衰）

### テキスト・効果
- 結婚式プリセット（Opening / プロフィール / Thank you / エンディング + ロワーサード 7 種・テロップ 8 種）
- テキスト詳細編集（縁取り・影・配置）
- テキストアニメーション（fade / slideUp / typewriter / scaleIn + MG 複合 4 種 + カスタムキーフレーム）
- トランジション11種（クロスフェード、ディゾルブ、ブラーディゾルブ、フェード、ワイプ、スライド、ズーム、アイリス）+ ミニプレビュー
...
- カラールックプリセット（結婚式向け 10 種ルック・クリップサムネのミニプレビュー）
- 映像クリップのフェードイン/アウト
...
- BGM/ナレーション音量正規化
- トランスフォームキーフレーム（位置・スケール・回転・不透明度・イージング・分割再配分・タイムライン UX）
- 音量キーフレームの splitClipAt 再配分
- J/K/L 再生ショートカット・リップルトリム
- ロワーサード/テロッププリセット拡充・BGM ビートマーカー
- カラールック 10 種・トランジション 11 種（dissolve 系追加）
- 速度キーフレーム（VideoClip、プレビュー・書き出し整合）
