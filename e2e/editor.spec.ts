...
test('ヘルプ: 婚礼制作フローの導線説明を表示できる', async ({ page }) => {
  await page.keyboard.press('?')
  await expect(page.getByRole('dialog', { name: 'ヘルプ' })).toBeVisible()
  await page.getByTestId('help-workflow-tab').click()
  await expect(page.getByTestId('help-workflow-content')).toBeVisible()
  await expect(page.getByText('テンプレ適用後の編集導線')).toBeVisible()
  await expect(page.getByText('章マーカーとは')).toBeVisible()
...
test('テンプレ導線: 構造化テンプレ適用後に編集ガイドを表示できる', async ({ page }) => {
  ...
  await expect(page.getByTestId('template-workflow-guide')).toBeVisible()
  await expect(page.getByText('テンプレ適用後の編集の流れ')).toBeVisible()
  await expect(page.getByText('素材を配置')).toBeVisible()

  await page.getByRole('button', { name: '了解' }).click()
  await expect(page.getByTestId('template-workflow-guide')).toBeHidden()
})

test('章マーカー: インスペクターに意味説明を表示する', async ({ page }) => {
  await page.getByTitle('テンプレ').click()
  await page.getByRole('button', { name: /結婚式フル構成/ }).click()
  await expect(page.getByText('結婚式フル構成テンプレートを適用しました')).toBeVisible()

  await page.locator('[title="オープニング"]').click()
  await expect(page.locator('aside').getByText('章マーカー', { exact: true })).toBeVisible()
  await expect(page.getByTestId('chapter-marker-guide')).toBeVisible()
  await expect(page.getByText('書き出しダイアログで章ごとの範囲 MP4')).toBeVisible()
})
...
test('書き出し: 章 ZIP 一括をキャンセルできる', async ({ page }) => {
  ...
  await expect(page.getByTestId('export-retry-button')).toContainText('残りの章から再開')
...
test('書き出し: キャンセル後に同じ設定で再試行できる（Phase H G23）', async ({ page }) => {
  test.setTimeout(60_000)
  ...
  await page.getByRole('button', { name: '1080p で書き出し' }).click()
  await page.getByRole('button', { name: 'キャンセル' }).click()
  await expect(page.getByTestId('export-retry-section')).toBeVisible()
  await page.getByTestId('export-retry-button').click()
  await expect(page.getByText(/% 完了/)).toBeVisible({ timeout: 15_000 })
  await page.getByRole('button', { name: 'キャンセル' }).click()
})

test('書き出し: 失敗後に同じ設定で再試行できる（Phase H G23）', async ({ page }) => {
  ...
  await page.getByTestId('export-retry-button').click()
  await expect(page.getByText('書き出しが完了しました')).toBeVisible({ timeout: 90_000 })
})

test('書き出し: 章 ZIP 失敗後に失敗章のみ再試行できる（Phase H G24）', async ({ page }) => {
  test.setTimeout(300_000)
  ...
  await expect(page.getByTestId('export-chapter-queue-error')).toContainText('新郎プロフィール')
...
test('書き出し: 章 ZIP キャンセル後に完了章を保持して再開できる（Phase H G26）', async ({ page }) => {
  test.setTimeout(300_000)
  ...
  await expect(page.getByRole('status', { name: '書き出し結果' })).toBeVisible()
  await expect(page.getByTestId('export-chapter-queue')).toBeVisible()
  await expect(page.getByText(/章が完了済みです/)).toBeVisible()
  await expect(page.getByTestId('export-retry-button')).toContainText('残りの章から再開')
...
test('書き出し: 章 ZIP キャンセル後に完了章を部分 ZIP 保存できる（Phase H G27）', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'キャンセル' }).click()
  await expect(page.getByRole('status', { name: '書き出し結果' })).toBeVisible()
  await expect(page.getByTestId('export-partial-zip-button')).toContainText(/完了した \d+ 章を ZIP で保存/)
...
test('書き出し: 章 ZIP 失敗後にスキップして残り章を続行できる（Phase H G28）', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '全章を ZIP で書き出し' }).click()
  await expect(page.getByRole('alert', { name: '書き出しエラー' })).toBeVisible({ timeout: 120_000 })
  await expect(page.getByTestId('export-skip-continue-button')).toContainText('失敗をスキップして続行')
...
test('書き出し: 失敗時自動スキップトグルで初回から続行できる（Phase H G29）', async ({ page }) => {
  test.setTimeout(300_000)
...
test('書き出し: 失敗時自動スキップトグルが再表示後も保持される（Phase H G29）', async ({ page }) => {
  await loadChapterExportE2eProject(page)
  await page.getByRole('button', { name: '書き出し' }).click()
  await page.getByTestId('export-continue-on-error-toggle').locator('input[type="checkbox"]').check()
  await page.keyboard.press('Escape')
...
test('書き出し: 失敗時に完了通知が送られる（Phase H G31）', async ({ page }) => {
  ...
  await expect(page.getByTestId('export-completion-notification-toggle').locator('input[type="checkbox"]')).toBeChecked()
  await setExportFailOnce(page)
  await page.getByRole('button', { name: '1080p で書き出し' }).click()
  await expect(page.getByRole('alert', { name: '書き出しエラー' })).toBeVisible({ timeout: 15_000 })
  ...
  expect(notes.some((n) => n.title.includes('失敗') || n.tag === 'fable-export-failure')).toBe(true)
})

test('書き出し: 完了通知トグルが再表示後も保持される（Phase H G31）', async ({ page }) => {
  await installExportNotificationMock(page)
  await addOpeningText(page)
  await page.getByRole('button', { name: '書き出し' }).click()
  await page.getByTestId('export-completion-notification-toggle').locator('input[type="checkbox"]').check()
...
test('書き出し: 失敗時に完了チャイムが鳴る（Phase H G32）', async ({ page }) => {
  test.setTimeout(90_000)

  const encodersSupported = await checkEncodersSupported(page)
  test.skip(!encodersSupported, 'エンコーダ非対応環境のためスキップ')

  await installExportChimeMock(page)
  await addOpeningText(page)
  await page.getByRole('button', { name: '書き出し' }).click()
  await page.getByTestId('export-completion-chime-toggle').locator('input[type="checkbox"]').check()
  await expect(page.getByTestId('export-completion-chime-toggle').locator('input[type="checkbox"]')).toBeChecked()
  await setExportFailOnce(page)
  await page.getByRole('button', { name: '1080p で書き出し' }).click()
  await expect(page.getByRole('alert', { name: '書き出しエラー' })).toBeVisible({ timeout: 15_000 })
  await expect.poll(async () => (await getExportChimes(page)).length).toBeGreaterThan(0)
  const chimes = await getExportChimes(page)
  expect(chimes).toContain('failure')
})

test('書き出し: 完了チャイムトグルが再表示後も保持される（Phase H G32）', async ({ page }) => {
  await installExportChimeMock(page)
  await addOpeningText(page)
  await page.getByRole('button', { name: '書き出し' }).click()
  await page.getByTestId('export-completion-chime-toggle').locator('input[type="checkbox"]').check()
...
test('書き出し: 4K長尺でメモリ警告が表示される', async ({ page }) => {
  const stats = await loadExportMemoryWarningStress(page)
  expect(stats.width).toBe(3840)
  expect(stats.height).toBe(2160)
  expect(stats.durationSec).toBeGreaterThanOrEqual(600)
  expect(stats.memoryLevel).toBe('high')

  await page.getByRole('button', { name: '書き出し' }).click()
  await expect(page.getByTestId('export-memory-warning')).toBeVisible()
  await expect(page.getByTestId('export-memory-warning')).toContainText('4K')
})

test('書き出し: 音声デコード失敗クリップを警告する（Phase G G22）', async ({ page }) => {
  const stats = await loadExportAudioDecodeStress(page)
  expect(stats.corruptAssetName).toBe('corrupt-bgm.bin')
  expect(stats.clipCount).toBeGreaterThanOrEqual(2)
  expect(stats.skippedAudioNames).toContain('corrupt-bgm.bin')
  ...
  await page.getByRole('button', { name: '1080p で書き出し' }).click()
  await expect(page.getByText('書き出しが完了しました')).toBeVisible({ timeout: 90_000 })
  await expect(page.getByText(/corrupt-bgm\.bin.*デコードに失敗/)).toBeVisible()
})
...
test('マグネティックタイムライン: プロジェクト設定で OFF にできる', async ({ page }) => {
  expect(await getMagneticTimeline(page)).toBe(true)

  await page.getByRole('button', { name: 'プロジェクト設定' }).click()
  await page.getByTestId('magnetic-timeline-toggle').uncheck()
  await page.getByRole('button', { name: 'キャンセル' }).click()

  expect(await getMagneticTimeline(page)).toBe(false)
  await expect(page.getByTestId('magnetic-timeline-indicator')).toHaveText('磁気 OFF')
})
...
test('速度ピッチ維持: タイムストレッチを有効化できる', async ({ page }) => {
  const stats = await loadSpeedAudioLinkStress(page)
  expect(await getVideoAudioPlaybackModeForClip(page, stats.videoClipId)).toBe('rate')

  await page.getByRole('button', { name: '再生速度' }).click()
  await page.getByTestId('speed-preserve-pitch-toggle').check()
  await expect(page.getByTestId('speed-audio-link-status')).toContainText('ピッチ維持')

  expect(await getVideoAudioPlaybackModeForClip(page, stats.videoClipId)).toBe('stretch')
  expect(await previewExportPlaybackModeParity(page, stats.videoClipId)).toBe(true)
  expect(await previewExportScheduleParity(page, stats.videoClipId, 0, stats.timelineDuration)).toBe(true)
})

test('速度ピッチ維持: OFF にすると playbackRate モードに戻る', async ({ page }) => {
  const stats = await loadSpeedAudioLinkStress(page)
  expect(await setSpeedPreservePitchById(page, stats.videoClipId, true)).toBe(true)
  expect(await getVideoAudioPlaybackModeForClip(page, stats.videoClipId)).toBe('stretch')

  await page.getByRole('button', { name: '再生速度' }).click()
  await page.getByTestId('speed-preserve-pitch-toggle').uncheck()
  expect(await getVideoAudioPlaybackModeForClip(page, stats.videoClipId)).toBe('rate')
})
