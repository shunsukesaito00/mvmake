...
import { makeSilentWav, mockNarrationRecordingScript } from './helpers'
...
test('メディア: ナレーション録音をプレビューしてタイムラインに配置できる', async ({ page, context }) => {
  await context.grantPermissions(['microphone'])
  await page.addInitScript(mockNarrationRecordingScript)
  await page.addInitScript(() => localStorage.setItem('fable-onboarded', '1'))
  await page.goto('./')
  await expect(page.getByText('FABLE', { exact: true })).toBeVisible()
  ...
  await expect(page.locator('footer').getByText(/^ナレーション_/)).toBeVisible()
})
