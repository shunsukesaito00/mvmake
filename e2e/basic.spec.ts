...
test('色調補正: LUT 付きユーザールックを組み込みルック適用後にユーザールックを適用した後に再適用した後の LUT 強度変更と LUT 切替と LUT「なし」選択を undo で LUT 選択とルック選択まで復元できる（ウエディング暖色再クリック挟み）', async ({ page }) => {
  await goOnboarded(page)
  await page.setInputFiles('input[accept*="image"]', { name: 'builtin-then-user-bundled-lut-intensity-switch-none-wedding-reclick-undo-photo.png', mimeType: 'image/png', buffer: TINY_PNG })
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'builtin-then-user-bundled-lut-intensity-switch-none-wedding-reclick-undo-photo.png')

  const weddingButton = page.getByRole('button', { name: 'ウエディング暖色ルック', exact: true })
  await weddingButton.click()
  await expect(weddingButton).toHaveAttribute('aria-pressed', 'true')

  await page.setInputFiles('input[accept*=".cube"]', { name: 'builtin-then-user-bundled-lut-intensity-switch-none-wedding-reclick-warm.cube', mimeType: 'text/plain', buffer: USER_LUT_UNDO_CUBE })
  await page.setInputFiles('input[accept*=".cube"]', { name: 'builtin-then-user-bundled-lut-intensity-switch-none-wedding-reclick-cool.cube', mimeType: 'text/plain', buffer: USER_LUT_SWITCH_CUBE_COOL })
  const lutSelect = page.getByLabel('LUT', { exact: true })
  await lutSelect.selectOption({ label: 'builtin-then-user-bundled-lut-intensity-switch-none-wedding-reclick-warm (2³)' })
  await page.getByRole('slider', { name: 'ミッドトーン' }).fill('0.2')
  await page.getByLabel('ルックプリセット名').fill('E2EBuiltinThenUserBundledLutIntensitySwitchNoneWeddingReclickUndo')
  await page.getByRole('button', { name: 'ルック保存' }).click()
  await expect(page.getByText('「E2EBuiltinThenUserBundledLutIntensitySwitchNoneWeddingReclickUndo」ルックを保存しました')).toBeVisible()

  const savedButton = page.getByRole('button', { name: 'E2EBuiltinThenUserBundledLutIntensitySwitchNoneWeddingReclickUndoルック', exact: true })
  await page.getByRole('button', { name: 'なしルック', exact: true }).click()
  await weddingButton.click()
  await expect(weddingButton).toHaveAttribute('aria-pressed', 'true')
  ...
  await lutSelect.selectOption({ label: 'builtin-then-user-bundled-lut-intensity-switch-none-wedding-reclick-cool (2³)' })
  await expect(lutSelect).not.toHaveValue(warmLutId)
...
test('色調補正: LUT 付きユーザールックを組み込みルック適用後にユーザールックを適用した後に再適用した後の LUT 強度変更と LUT 切替を undo で LUT 選択とルック選択まで復元できる（ウエディング暖色2回再クリック挟み）', async ({ page }) => {
  await goOnboarded(page)
  await page.setInputFiles('input[accept*="image"]', { name: 'builtin-then-user-bundled-lut-intensity-switch-wedding-double-reclick-undo-photo.png', mimeType: 'image/png', buffer: TINY_PNG })
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'builtin-then-user-bundled-lut-intensity-switch-wedding-double-reclick-undo-photo.png')

  const weddingButton = page.getByRole('button', { name: 'ウエディング暖色ルック', exact: true })
  await weddingButton.click()
  await expect(weddingButton).toHaveAttribute('aria-pressed', 'true')

  await page.setInputFiles('input[accept*=".cube"]', { name: 'builtin-then-user-bundled-lut-intensity-switch-wedding-double-reclick-warm.cube', mimeType: 'text/plain', buffer: USER_LUT_UNDO_CUBE })
  await page.setInputFiles('input[accept*=".cube"]', { name: 'builtin-then-user-bundled-lut-intensity-switch-wedding-double-reclick-cool.cube', mimeType: 'text/plain', buffer: USER_LUT_SWITCH_CUBE_COOL })
  const lutSelect = page.getByLabel('LUT', { exact: true })
  await lutSelect.selectOption({ label: 'builtin-then-user-bundled-lut-intensity-switch-wedding-double-reclick-warm (2³)' })
  await page.getByRole('slider', { name: 'ミッドトーン' }).fill('0.2')
  await page.getByLabel('ルックプリセット名').fill('E2EBuiltinThenUserBundledLutIntensitySwitchWeddingDoubleReclickUndo')
  await page.getByRole('button', { name: 'ルック保存' }).click()
  await expect(page.getByText('「E2EBuiltinThenUserBundledLutIntensitySwitchWeddingDoubleReclickUndo」ルックを保存しました')).toBeVisible()

  const savedButton = page.getByRole('button', { name: 'E2EBuiltinThenUserBundledLutIntensitySwitchWeddingDoubleReclickUndoルック', exact: true })
  await page.getByRole('button', { name: 'なしルック', exact: true }).click()
  await weddingButton.click()
  await expect(weddingButton).toHaveAttribute('aria-pressed', 'true')
  ...
  await lutSelect.selectOption({ label: 'builtin-then-user-bundled-lut-intensity-switch-wedding-double-reclick-cool (2³)' })
  await expect(lutSelect).not.toHaveValue(warmLutId)
...
test('色調補正: LUT 付きユーザールックを組み込みルック適用後にユーザールックを適用した後に再適用した後の LUT 強度変更と LUT「なし」選択を undo で LUT 選択とルック選択まで復元できる（ウエディング暖色2回再クリック挟み）', async ({ page }) => {
  await goOnboarded(page)
  await page.setInputFiles('input[accept*="image"]', { name: 'builtin-then-user-bundled-lut-intensity-none-wedding-double-reclick-undo-photo.png', mimeType: 'image/png', buffer: TINY_PNG })
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'builtin-then-user-bundled-lut-intensity-none-wedding-double-reclick-undo-photo.png')

  const weddingButton = page.getByRole('button', { name: 'ウエディング暖色ルック', exact: true })
  await weddingButton.click()
  await expect(weddingButton).toHaveAttribute('aria-pressed', 'true')

  await page.setInputFiles('input[accept*=".cube"]', { name: 'builtin-then-user-bundled-lut-intensity-none-wedding-double-reclick-warm.cube', mimeType: 'text/plain', buffer: USER_LUT_UNDO_CUBE })
  const lutSelect = page.getByLabel('LUT', { exact: true })
  await lutSelect.selectOption({ label: 'builtin-then-user-bundled-lut-intensity-none-wedding-double-reclick-warm (2³)' })
  await page.getByRole('slider', { name: 'ミッドトーン' }).fill('0.2')
  await page.getByLabel('ルックプリセット名').fill('E2EBuiltinThenUserBundledLutIntensityNoneWeddingDoubleReclickUndo')
  await page.getByRole('button', { name: 'ルック保存' }).click()
  await expect(page.getByText('「E2EBuiltinThenUserBundledLutIntensityNoneWeddingDoubleReclickUndo」ルックを保存しました')).toBeVisible()

  const savedButton = page.getByRole('button', { name: 'E2EBuiltinThenUserBundledLutIntensityNoneWeddingDoubleReclickUndoルック', exact: true })
  await page.getByRole('button', { name: 'なしルック', exact: true }).click()
  await weddingButton.click()
  await expect(weddingButton).toHaveAttribute('aria-pressed', 'true')
...
test('色調補正: LUT 付きユーザールックを組み込みルック適用後にユーザールックを適用した後に再適用した後の LUT 強度変更と LUT 切替を undo で LUT 選択とルック選択まで復元できる（組み込みルック2回再クリック挟み）', async ({ page }) => {
  await goOnboarded(page)
  await page.setInputFiles('input[accept*="image"]', { name: 'builtin-then-user-bundled-lut-intensity-switch-builtin-double-reclick-undo-photo.png', mimeType: 'image/png', buffer: TINY_PNG })
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'builtin-then-user-bundled-lut-intensity-switch-builtin-double-reclick-undo-photo.png')

  const filmButton = page.getByRole('button', { name: 'フィルム風ルック', exact: true })
  await filmButton.click()
  await expect(filmButton).toHaveAttribute('aria-pressed', 'true')

  await page.setInputFiles('input[accept*=".cube"]', { name: 'builtin-then-user-bundled-lut-intensity-switch-builtin-double-reclick-warm.cube', mimeType: 'text/plain', buffer: USER_LUT_UNDO_CUBE })
  await page.setInputFiles('input[accept*=".cube"]', { name: 'builtin-then-user-bundled-lut-intensity-switch-builtin-double-reclick-cool.cube', mimeType: 'text/plain', buffer: USER_LUT_SWITCH_CUBE_COOL })
  const lutSelect = page.getByLabel('LUT', { exact: true })
  await lutSelect.selectOption({ label: 'builtin-then-user-bundled-lut-intensity-switch-builtin-double-reclick-warm (2³)' })
  await page.getByRole('slider', { name: 'ミッドトーン' }).fill('0.2')
  await page.getByLabel('ルックプリセット名').fill('E2EBuiltinThenUserBundledLutIntensitySwitchBuiltinDoubleReclickUndo')
  await page.getByRole('button', { name: 'ルック保存' }).click()
  await expect(page.getByText('「E2EBuiltinThenUserBundledLutIntensitySwitchBuiltinDoubleReclickUndo」ルックを保存しました')).toBeVisible()

  const savedButton = page.getByRole('button', { name: 'E2EBuiltinThenUserBundledLutIntensitySwitchBuiltinDoubleReclickUndoルック', exact: true })
  await page.getByRole('button', { name: 'なしルック', exact: true }).click()
  await filmButton.click()
  await expect(filmButton).toHaveAttribute('aria-pressed', 'true')
  ...
  await lutSelect.selectOption({ label: 'builtin-then-user-bundled-lut-intensity-switch-builtin-double-reclick-cool (2³)' })
  await expect(lutSelect).not.toHaveValue(warmLutId)
...
test('色調補正: LUT 付きユーザールックを組み込みルック適用後にユーザールックを適用した後に再適用した後の LUT 強度変更と LUT「なし」選択を undo で LUT 選択とルック選択まで復元できる（組み込みルック2回再クリック挟み）', async ({ page }) => {
  await goOnboarded(page)
  await page.setInputFiles('input[accept*="image"]', { name: 'builtin-then-user-bundled-lut-intensity-none-builtin-double-reclick-undo-photo.png', mimeType: 'image/png', buffer: TINY_PNG })
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'builtin-then-user-bundled-lut-intensity-none-builtin-double-reclick-undo-photo.png')

  const filmButton = page.getByRole('button', { name: 'フィルム風ルック', exact: true })
  await filmButton.click()
  await expect(filmButton).toHaveAttribute('aria-pressed', 'true')

  await page.setInputFiles('input[accept*=".cube"]', { name: 'builtin-then-user-bundled-lut-intensity-none-builtin-double-reclick-warm.cube', mimeType: 'text/plain', buffer: USER_LUT_UNDO_CUBE })
  const lutSelect = page.getByLabel('LUT', { exact: true })
  await lutSelect.selectOption({ label: 'builtin-then-user-bundled-lut-intensity-none-builtin-double-reclick-warm (2³)' })
  await page.getByRole('slider', { name: 'ミッドトーン' }).fill('0.2')
  await page.getByLabel('ルックプリセット名').fill('E2EBuiltinThenUserBundledLutIntensityNoneBuiltinDoubleReclickUndo')
  await page.getByRole('button', { name: 'ルック保存' }).click()
  await expect(page.getByText('「E2EBuiltinThenUserBundledLutIntensityNoneBuiltinDoubleReclickUndo」ルックを保存しました')).toBeVisible()

  const savedButton = page.getByRole('button', { name: 'E2EBuiltinThenUserBundledLutIntensityNoneBuiltinDoubleReclickUndoルック', exact: true })
  await page.getByRole('button', { name: 'なしルック', exact: true }).click()
  await filmButton.click()
  await expect(filmButton).toHaveAttribute('aria-pressed', 'true')
