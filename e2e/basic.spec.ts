...
test('色調補正: LUT 付きユーザールックを組み込みルック適用後にユーザールックを適用した後に再適用した後の LUT 強度変更と LUT 切替を undo で LUT 選択とルック選択まで復元できる（フィルム風再クリック挟み・film-reclick）', async ({ page }) => {
  await goOnboarded(page)
...
test('色調補正: LUT 付きユーザールックを組み込みルック適用後にユーザールックを適用した後に再適用した後の LUT 強度変更と LUT「なし」選択を undo で LUT 選択とルック選択まで復元できる（フィルム風再クリック挟み・film-reclick）', async ({ page }) => {
  await goOnboarded(page)
...
test('色調補正: LUT 付きユーザールックを組み込みルック適用後にユーザールックを適用した後に再適用した後の LUT 強度変更と LUT 切替と LUT「なし」選択を undo で LUT 選択とルック選択まで復元できる（フィルム風再クリック挟み・film-reclick）', async ({ page }) => {
  await goOnboarded(page)
...
test('テキスト: スライドアップアニメーションを設定して長さを変更できる', async ({ page }) => {
  await goOnboarded(page)
  await addOpeningText(page)
  await clickTimelineClip(page, 'Opening')
...
test('テキスト: タイプライターアニメーションを設定できる', async ({ page }) => {
  await goOnboarded(page)
  await addOpeningText(page)
  await clickTimelineClip(page, 'Opening')
...
test('テキスト: スケールインアニメーションを設定できる', async ({ page }) => {
  await goOnboarded(page)
  await addOpeningText(page)
  await clickTimelineClip(page, 'Opening')
...
test('トランジション: ワイプを画像クリップに適用できる', async ({ page }) => {
  await goOnboarded(page)
  await page.getByTitle('メディア').click()
  await page.setInputFiles('input[accept*="image"]', [
    { name: 'wipe-a.png', mimeType: 'image/png', buffer: TINY_PNG },
    { name: 'wipe-b.png', mimeType: 'image/png', buffer: TINY_PNG },
  ])
  await expect(page.getByText('2件のメディアを追加しました')).toBeVisible()
  await page.locator('button[title="クリックで再生位置に追加"]').filter({ hasText: 'wipe-a.png' }).click()
  await page.locator('button[title="クリックで再生位置に追加"]').filter({ hasText: 'wipe-b.png' }).click()

  await clickTimelineClip(page, 'wipe-b.png')
  await page.getByTitle('効果').click()
  await page.getByRole('button', { name: 'ワイプ', exact: true }).click()
  await expect(page.getByText('ワイプを適用しました')).toBeVisible()
})

test('トランジション: スライド右を画像クリップに適用できる', async ({ page }) => {
  await goOnboarded(page)
  await page.getByTitle('メディア').click()
  await page.setInputFiles('input[accept*="image"]', [
    { name: 'slide-right-a.png', mimeType: 'image/png', buffer: TINY_PNG },
    { name: 'slide-right-b.png', mimeType: 'image/png', buffer: TINY_PNG },
  ])
  await expect(page.getByText('2件のメディアを追加しました')).toBeVisible()
  await page.locator('button[title="クリックで再生位置に追加"]').filter({ hasText: 'slide-right-a.png' }).click()
  await page.locator('button[title="クリックで再生位置に追加"]').filter({ hasText: 'slide-right-b.png' }).click()

  await clickTimelineClip(page, 'slide-right-b.png')
  await page.getByTitle('効果').click()
  await page.getByRole('button', { name: 'スライド右', exact: true }).click()
  await expect(page.getByText('スライド右を適用しました')).toBeVisible()
})

test('テキスト: アニメーションなしに切り替えできる', async ({ page }) => {
  await goOnboarded(page)
  await addOpeningText(page)
  await clickTimelineClip(page, 'Opening')
...
test('インスペクター: 画像クリップの Ken Burns 終了スケールを変更できる', async ({ page }) => {
  await goOnboarded(page)
  await page.setInputFiles('input[accept*="image"]', { name: 'ken-burns-end.png', mimeType: 'image/png', buffer: TINY_PNG })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible()
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'ken-burns-end.png')

  await page.getByRole('button', { name: 'Ken Burns', exact: true }).click()
  const endScaleSlider = page.getByRole('slider', { name: '終了スケール' })
  await endScaleSlider.fill('1.6')
  await expect(endScaleSlider).toHaveValue('1.6')
})

test('インスペクター: 画像クリップにフェードアウトを設定できる', async ({ page }) => {
  await goOnboarded(page)
  await page.setInputFiles('input[accept*="image"]', { name: 'fade-out-photo.png', mimeType: 'image/png', buffer: TINY_PNG })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible()
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'fade-out-photo.png')

  await page.getByRole('button', { name: 'フェード' }).click()
  const fadeOut = page.getByRole('slider', { name: 'フェードアウト' })
  await fadeOut.fill('0.8')
  await expect(fadeOut).toHaveValue('0.8')
})

test('インスペクター: テキストクリップの長さを変更できる', async ({ page }) => {
  await goOnboarded(page)
  await addOpeningText(page)
  await clickTimelineClip(page, 'Opening')
...
test('インスペクター: 動画クリップのトランスフォームキーフレームのスケールを数値入力できる', async ({ page }) => {
  await goOnboarded(page)
  const webm = await makeTinyWebmVideo(page)
  await page.getByTitle('メディア').click()
  await page.setInputFiles('input[accept*="video"]', { name: 'video-tf-scale.webm', mimeType: 'video/webm', buffer: webm })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible({ timeout: 15_000 })
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'video-tf-scale.webm')
...
test('インスペクター: 画像クリップのトランスフォームキーフレームのスケールを数値入力できる', async ({ page }) => {
  await goOnboarded(page)
  await page.setInputFiles('input[accept*="image"]', { name: 'image-tf-scale.png', mimeType: 'image/png', buffer: TINY_PNG })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible()
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'image-tf-scale.png')
...
test('インスペクター: 動画クリップのトランスフォームキーフレームのグラフエディターを表示できる', async ({ page }) => {
  await goOnboarded(page)
  const webm = await makeTinyWebmVideo(page)
  await page.getByTitle('メディア').click()
  await page.setInputFiles('input[accept*="video"]', { name: 'video-tf-graph.webm', mimeType: 'video/webm', buffer: webm })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible({ timeout: 15_000 })
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'video-tf-graph.webm')
...
test('インスペクター: 動画クリップのトランスフォームキーフレームのグラフエディターで回転プロパティを切り替えできる', async ({ page }) => {
  await goOnboarded(page)
  const webm = await makeTinyWebmVideo(page)
  await page.getByTitle('メディア').click()
  await page.setInputFiles('input[accept*="video"]', { name: 'video-tf-graph-rotation.webm', mimeType: 'video/webm', buffer: webm })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible({ timeout: 15_000 })
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'video-tf-graph-rotation.webm')
...
test('インスペクター: テキストクリップのトランスフォームキーフレームのグラフエディターで回転プロパティを切り替えできる', async ({ page }) => {
  await goOnboarded(page)
  await addOpeningText(page)
  await clickTimelineClip(page, 'Opening')
...
test('インスペクター: 動画クリップのトランスフォームキーフレームのグラフエディターでスケールプロパティを切り替えできる', async ({ page }) => {
  await goOnboarded(page)
  const webm = await makeTinyWebmVideo(page)
  await page.getByTitle('メディア').click()
  await page.setInputFiles('input[accept*="video"]', { name: 'video-tf-graph-scale.webm', mimeType: 'video/webm', buffer: webm })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible({ timeout: 15_000 })
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'video-tf-graph-scale.webm')
  ...
  await page.getByTestId('transform-graph-property-scale').click()
  await expect(page.getByTestId('transform-graph-property-scale')).toHaveAttribute('aria-pressed', 'true')
  await expect(page.getByTestId('transform-graph-property-rotation')).toHaveAttribute('aria-pressed', 'false')
})

test('インスペクター: 画像クリップのトランスフォームキーフレームのグラフエディターでスケールプロパティを切り替えできる', async ({ page }) => {
  await goOnboarded(page)
  await page.setInputFiles('input[accept*="image"]', { name: 'image-tf-graph-scale.png', mimeType: 'image/png', buffer: TINY_PNG })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible()
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'image-tf-graph-scale.png')
...
test('インスペクター: テキストクリップのトランスフォームキーフレームのグラフエディターでスケールプロパティを切り替えできる', async ({ page }) => {
  await goOnboarded(page)
  await addOpeningText(page)
  await clickTimelineClip(page, 'Opening')
...
test('インスペクター: 動画クリップのトランスフォームキーフレームのグラフエディターで不透明度プロパティを切り替えできる', async ({ page }) => {
  await goOnboarded(page)
  const webm = await makeTinyWebmVideo(page)
  await page.getByTitle('メディア').click()
  await page.setInputFiles('input[accept*="video"]', { name: 'video-tf-graph-opacity.webm', mimeType: 'video/webm', buffer: webm })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible({ timeout: 15_000 })
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'video-tf-graph-opacity.webm')
...
test('インスペクター: 画像クリップのトランスフォームキーフレームのグラフエディターでXプロパティを切り替えできる', async ({ page }) => {
  await goOnboarded(page)
  await page.setInputFiles('input[accept*="image"]', { name: 'image-tf-graph-x.png', mimeType: 'image/png', buffer: TINY_PNG })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible()
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'image-tf-graph-x.png')
...
test('インスペクター: テキストクリップのトランスフォームキーフレームのグラフエディターでXプロパティを切り替えできる', async ({ page }) => {
  await goOnboarded(page)
  await addOpeningText(page)
  await clickTimelineClip(page, 'Opening')
...
test('インスペクター: 動画クリップのトランスフォームキーフレームのグラフエディターでYプロパティを切り替えできる', async ({ page }) => {
  await goOnboarded(page)
  const webm = await makeTinyWebmVideo(page)
  await page.getByTitle('メディア').click()
  await page.setInputFiles('input[accept*="video"]', { name: 'video-tf-graph-y.webm', mimeType: 'video/webm', buffer: webm })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible({ timeout: 15_000 })
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'video-tf-graph-y.webm')
...
test('インスペクター: 画像クリップのトランスフォームキーフレームのグラフエディターでYプロパティを切り替えできる', async ({ page }) => {
  await goOnboarded(page)
  await page.setInputFiles('input[accept*="image"]', { name: 'image-tf-graph-y.png', mimeType: 'image/png', buffer: TINY_PNG })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible()
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'image-tf-graph-y.png')
...
test('インスペクター: テキストクリップのトランスフォームキーフレームのグラフエディターでYプロパティを切り替えできる', async ({ page }) => {
  await goOnboarded(page)
  await addOpeningText(page)
  await clickTimelineClip(page, 'Opening')
...
test('インスペクター: 動画クリップのトランスフォームキーフレームのグラフエディターでキーフレームを選択できる', async ({ page }) => {
  await goOnboarded(page)
  const webm = await makeTinyWebmVideo(page)
  await page.getByTitle('メディア').click()
  await page.setInputFiles('input[accept*="video"]', { name: 'video-tf-graph-select.webm', mimeType: 'video/webm', buffer: webm })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible({ timeout: 15_000 })
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'video-tf-graph-select.webm')
  ...
  await expect(page.getByTestId('transform-kf-graph-editor')).toBeVisible()
  await page.getByRole('button', { name: 'グラフ上のキーフレーム 1' }).click()
  await expect(page.locator('[class*="ring-sky-400"]').filter({ hasText: 'キーフレーム 1' })).toBeVisible()
  await page.getByRole('button', { name: 'グラフ上のキーフレーム 2' }).click()
  await expect(page.locator('[class*="ring-sky-400"]').filter({ hasText: 'キーフレーム 2' })).toBeVisible()
})

test('インスペクター: 画像クリップのトランスフォームキーフレームのグラフエディターでキーフレームを選択できる', async ({ page }) => {
  await goOnboarded(page)
  await page.setInputFiles('input[accept*="image"]', { name: 'image-tf-graph-select.png', mimeType: 'image/png', buffer: TINY_PNG })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible()
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'image-tf-graph-select.png')
...
test('インスペクター: テキストクリップのトランスフォームキーフレームのグラフエディターでキーフレームを選択できる', async ({ page }) => {
  await goOnboarded(page)
  await addOpeningText(page)
  await clickTimelineClip(page, 'Opening')
...
test('インスペクター: 動画クリップのトランスフォームキーフレームのグラフエディターで2点以上のキーフレームカーブを表示できる', async ({ page }) => {
  await goOnboarded(page)
  const webm = await makeTinyWebmVideo(page)
  await page.getByTitle('メディア').click()
  await page.setInputFiles('input[accept*="video"]', { name: 'video-tf-graph-curve.webm', mimeType: 'video/webm', buffer: webm })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible({ timeout: 15_000 })
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'video-tf-graph-curve.webm')
...
test('インスペクター: 画像クリップのトランスフォームキーフレームのグラフエディターで3点のキーフレームを表示できる', async ({ page }) => {
  await goOnboarded(page)
  await page.setInputFiles('input[accept*="image"]', { name: 'image-tf-graph-3kf.png', mimeType: 'image/png', buffer: TINY_PNG })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible()
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'image-tf-graph-3kf.png')
...
test('インスペクター: テキストクリップのトランスフォームキーフレームのグラフエディターで3点のキーフレームを表示できる', async ({ page }) => {
  await goOnboarded(page)
  await addOpeningText(page)
  await clickTimelineClip(page, 'Opening')
...
test('インスペクター: 動画クリップのトランスフォームキーフレームのグラフエディターでキーフレームを削除できる', async ({ page }) => {
  await goOnboarded(page)
  const webm = await makeTinyWebmVideo(page)
  await page.getByTitle('メディア').click()
  await page.setInputFiles('input[accept*="video"]', { name: 'video-tf-graph-delete.webm', mimeType: 'video/webm', buffer: webm })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible({ timeout: 15_000 })
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'video-tf-graph-delete.webm')
  ...
  await expect(page.getByRole('button', { name: 'グラフ上のキーフレーム 2' })).toHaveCount(0)
  await expect(page.getByRole('button', { name: 'グラフ上のキーフレーム 1' })).toBeVisible()
  await expect(graph.locator('path')).toHaveCount(1)
})

test('インスペクター: 画像クリップのトランスフォームキーフレームのグラフエディターでキーフレームを削除できる', async ({ page }) => {
  await goOnboarded(page)
  await page.setInputFiles('input[accept*="image"]', { name: 'image-tf-graph-delete.png', mimeType: 'image/png', buffer: TINY_PNG })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible()
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'image-tf-graph-delete.png')
...
test('インスペクター: テキストクリップのトランスフォームキーフレームのグラフエディターでキーフレームを削除できる', async ({ page }) => {
  await goOnboarded(page)
  await addOpeningText(page)
  await clickTimelineClip(page, 'Opening')
...
test('インスペクター: 動画クリップのトランスフォームキーフレームのグラフエディターで最後のキーフレームを削除するとグラフが非表示になる', async ({ page }) => {
  await goOnboarded(page)
  const webm = await makeTinyWebmVideo(page)
  await page.getByTitle('メディア').click()
  await page.setInputFiles('input[accept*="video"]', { name: 'video-tf-graph-delete-last.webm', mimeType: 'video/webm', buffer: webm })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible({ timeout: 15_000 })
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'video-tf-graph-delete-last.webm')
  ...
  const graph = page.getByTestId('transform-kf-graph-editor')
  await expect(graph).toBeVisible()
  await page.locator('[class*="ring-sky-400"]').filter({ hasText: 'キーフレーム 1' }).getByRole('button', { name: '削除' }).click()

  await expect(graph).toBeHidden()
  await expect(page.getByText('再生ヘッド位置(クリップ内)に位置・スケール・回転・不透明度のキーフレームを追加できます')).toBeVisible()
})

test('インスペクター: 画像クリップのトランスフォームキーフレームのグラフエディターで最後のキーフレームを削除するとグラフが非表示になる', async ({ page }) => {
  await goOnboarded(page)
  await page.setInputFiles('input[accept*="image"]', { name: 'image-tf-graph-delete-last.png', mimeType: 'image/png', buffer: TINY_PNG })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible()
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'image-tf-graph-delete-last.png')
...
test('インスペクター: テキストクリップのトランスフォームキーフレームのグラフエディターで最後のキーフレームを削除するとグラフが非表示になる', async ({ page }) => {
  await goOnboarded(page)
  await addOpeningText(page)
  await clickTimelineClip(page, 'Opening')
...
test('インスペクター: 動画クリップのトランスフォームキーフレームのグラフエディターで2点のキーフレームをすべて削除するとグラフが非表示になる', async ({ page }) => {
  await goOnboarded(page)
  const webm = await makeTinyWebmVideo(page)
  await page.getByTitle('メディア').click()
  await page.setInputFiles('input[accept*="video"]', { name: 'video-tf-graph-delete-all.webm', mimeType: 'video/webm', buffer: webm })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible({ timeout: 15_000 })
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'video-tf-graph-delete-all.webm')
...
test('インスペクター: 画像クリップのトランスフォームキーフレームのグラフエディターで2点のキーフレームをすべて削除するとグラフが非表示になる', async ({ page }) => {
  await goOnboarded(page)
  await page.setInputFiles('input[accept*="image"]', { name: 'image-tf-graph-delete-all.png', mimeType: 'image/png', buffer: TINY_PNG })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible()
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'image-tf-graph-delete-all.png')
...
test('インスペクター: テキストクリップのトランスフォームキーフレームのグラフエディターで2点のキーフレームをすべて削除するとグラフが非表示になる', async ({ page }) => {
  await goOnboarded(page)
  await addOpeningText(page)
  await clickTimelineClip(page, 'Opening')
...
test('インスペクター: 動画クリップのトランスフォームキーフレームのグラフエディターでキーフレーム削除後に再追加するとグラフが再表示される', async ({ page }) => {
  await goOnboarded(page)
  const webm = await makeTinyWebmVideo(page)
  await page.getByTitle('メディア').click()
  await page.setInputFiles('input[accept*="video"]', { name: 'video-tf-graph-readd.webm', mimeType: 'video/webm', buffer: webm })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible({ timeout: 15_000 })
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'video-tf-graph-readd.webm')
...
test('インスペクター: 画像クリップのトランスフォームキーフレームのグラフエディターでキーフレーム削除後に再追加するとグラフが再表示される', async ({ page }) => {
  await goOnboarded(page)
  await page.setInputFiles('input[accept*="image"]', { name: 'image-tf-graph-readd.png', mimeType: 'image/png', buffer: TINY_PNG })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible()
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'image-tf-graph-readd.png')
...
test('インスペクター: テキストクリップのトランスフォームキーフレームのグラフエディターでキーフレーム削除後に再追加するとグラフが再表示される', async ({ page }) => {
  await goOnboarded(page)
  await addOpeningText(page)
  await clickTimelineClip(page, 'Opening')
...
test('インスペクター: 動画クリップのトランスフォームキーフレームのグラフエディターで3点のキーフレームをすべて削除するとグラフが非表示になる', async ({ page }) => {
  await goOnboarded(page)
  const webm = await makeTinyWebmVideo(page)
  await page.getByTitle('メディア').click()
  await page.setInputFiles('input[accept*="video"]', { name: 'video-tf-graph-delete-3kf.webm', mimeType: 'video/webm', buffer: webm })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible({ timeout: 15_000 })
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'video-tf-graph-delete-3kf.webm')
...
test('インスペクター: 画像クリップのトランスフォームキーフレームのグラフエディターで3点のキーフレームをすべて削除するとグラフが非表示になる', async ({ page }) => {
  await goOnboarded(page)
  await page.setInputFiles('input[accept*="image"]', { name: 'image-tf-graph-delete-3kf.png', mimeType: 'image/png', buffer: TINY_PNG })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible()
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'image-tf-graph-delete-3kf.png')
...
test('インスペクター: テキストクリップのトランスフォームキーフレームのグラフエディターで3点のキーフレームをすべて削除するとグラフが非表示になる', async ({ page }) => {
  await goOnboarded(page)
  await addOpeningText(page)
  await clickTimelineClip(page, 'Opening')
...
test('インスペクター: 動画クリップのトランスフォームキーフレームのグラフエディターでキーフレーム削除後に2点再追加するとカーブを表示できる', async ({ page }) => {
  await goOnboarded(page)
  const webm = await makeTinyWebmVideo(page)
  await page.getByTitle('メディア').click()
  await page.setInputFiles('input[accept*="video"]', { name: 'video-tf-graph-readd-curve.webm', mimeType: 'video/webm', buffer: webm })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible({ timeout: 15_000 })
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'video-tf-graph-readd-curve.webm')
...
test('インスペクター: 画像クリップのトランスフォームキーフレームのグラフエディターでキーフレーム削除後に2点再追加するとカーブを表示できる', async ({ page }) => {
  await goOnboarded(page)
  await page.setInputFiles('input[accept*="image"]', { name: 'image-tf-graph-readd-curve.png', mimeType: 'image/png', buffer: TINY_PNG })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible()
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'image-tf-graph-readd-curve.png')
...
test('インスペクター: テキストクリップのトランスフォームキーフレームのグラフエディターでキーフレーム削除後に2点再追加するとカーブを表示できる', async ({ page }) => {
  await goOnboarded(page)
  await addOpeningText(page)
  await clickTimelineClip(page, 'Opening')
...
test('インスペクター: 動画クリップのトランスフォームキーフレームのグラフエディターで3点のキーフレームをすべて削除後に再追加するとグラフが再表示される', async ({ page }) => {
  await goOnboarded(page)
  const webm = await makeTinyWebmVideo(page)
  await page.getByTitle('メディア').click()
  await page.setInputFiles('input[accept*="video"]', { name: 'video-tf-graph-delete-3kf-readd.webm', mimeType: 'video/webm', buffer: webm })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible({ timeout: 15_000 })
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'video-tf-graph-delete-3kf-readd.webm')
...
test('インスペクター: 画像クリップのトランスフォームキーフレームのグラフエディターで3点のキーフレームをすべて削除後に再追加するとグラフが再表示される', async ({ page }) => {
  await goOnboarded(page)
  await page.setInputFiles('input[accept*="image"]', { name: 'image-tf-graph-delete-3kf-readd.png', mimeType: 'image/png', buffer: TINY_PNG })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible()
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'image-tf-graph-delete-3kf-readd.png')
...
test('インスペクター: テキストクリップのトランスフォームキーフレームのグラフエディターで3点のキーフレームをすべて削除後に再追加するとグラフが再表示される', async ({ page }) => {
  await goOnboarded(page)
  await addOpeningText(page)
  await clickTimelineClip(page, 'Opening')
...
test('インスペクター: 動画クリップのトランスフォームキーフレームのグラフエディターでキーフレーム追加時に件数表示が更新される', async ({ page }) => {
  await goOnboarded(page)
  const webm = await makeTinyWebmVideo(page)
  await page.getByTitle('メディア').click()
  await page.setInputFiles('input[accept*="video"]', { name: 'video-tf-graph-kf-count.webm', mimeType: 'video/webm', buffer: webm })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible({ timeout: 15_000 })
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'video-tf-graph-kf-count.webm')
...
test('インスペクター: 画像クリップのトランスフォームキーフレームのグラフエディターでキーフレーム削除時に件数表示が更新される', async ({ page }) => {
  await goOnboarded(page)
  await page.setInputFiles('input[accept*="image"]', { name: 'image-tf-graph-kf-count-delete.png', mimeType: 'image/png', buffer: TINY_PNG })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible()
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'image-tf-graph-kf-count-delete.png')
...
test('インスペクター: テキストクリップのトランスフォームキーフレームのグラフエディターでキーフレーム削除時に件数表示が更新される', async ({ page }) => {
  await goOnboarded(page)
  await addOpeningText(page)
  await clickTimelineClip(page, 'Opening')
...
test('インスペクター: 画像クリップのトランスフォームキーフレームのグラフエディターで3点のキーフレームをすべて削除後に2点再追加するとカーブを表示できる', async ({ page }) => {
  await goOnboarded(page)
  await page.setInputFiles('input[accept*="image"]', { name: 'image-tf-graph-delete-3kf-readd-curve.png', mimeType: 'image/png', buffer: TINY_PNG })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible()
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'image-tf-graph-delete-3kf-readd-curve.png')
...
test('インスペクター: テキストクリップのトランスフォームキーフレームのグラフエディターで3点のキーフレームをすべて削除後に2点再追加するとカーブを表示できる', async ({ page }) => {
  await goOnboarded(page)
  await addOpeningText(page)
  await clickTimelineClip(page, 'Opening')
...
test('インスペクター: 動画クリップのトランスフォームキーフレームのグラフエディターで3点のキーフレームをすべて削除後に2点再追加するとカーブを表示できる', async ({ page }) => {
  await goOnboarded(page)
  const webm = await makeTinyWebmVideo(page)
  await page.getByTitle('メディア').click()
  await page.setInputFiles('input[accept*="video"]', { name: 'video-tf-graph-delete-3kf-readd-curve.webm', mimeType: 'video/webm', buffer: webm })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible({ timeout: 15_000 })
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'video-tf-graph-delete-3kf-readd-curve.webm')
...
test('インスペクター: 動画クリップのトランスフォームキーフレームのグラフエディターで最終キーフレーム削除後に件数表示が非表示になる', async ({ page }) => {
  await goOnboarded(page)
  const webm = await makeTinyWebmVideo(page)
  await page.getByTitle('メディア').click()
  await page.setInputFiles('input[accept*="video"]', { name: 'video-tf-graph-kf-count-last-delete.webm', mimeType: 'video/webm', buffer: webm })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible({ timeout: 15_000 })
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'video-tf-graph-kf-count-last-delete.webm')
  ...
  await expect(page.getByText('1件', { exact: true })).toBeHidden()
  await expect(page.getByText('再生ヘッド位置(クリップ内)に位置・スケール・回転・不透明度のキーフレームを追加できます')).toBeVisible()
  await expect(graph).toBeHidden()
})

test('インスペクター: 画像クリップのトランスフォームキーフレームのグラフエディターで最終キーフレーム削除後に件数表示が非表示になる', async ({ page }) => {
  await goOnboarded(page)
  await page.setInputFiles('input[accept*="image"]', { name: 'image-tf-graph-kf-count-last-delete.png', mimeType: 'image/png', buffer: TINY_PNG })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible()
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'image-tf-graph-kf-count-last-delete.png')
...
test('インスペクター: テキストクリップのトランスフォームキーフレームのグラフエディターで最終キーフレーム削除後に件数表示が非表示になる', async ({ page }) => {
  await goOnboarded(page)
  await addOpeningText(page)
  await clickTimelineClip(page, 'Opening')
...
test('インスペクター: 動画クリップのトランスフォームキーフレームのグラフエディターでキーフレーム0件時に件数表示が0件になる', async ({ page }) => {
  await goOnboarded(page)
  const webm = await makeTinyWebmVideo(page)
  await page.getByTitle('メディア').click()
  await page.setInputFiles('input[accept*="video"]', { name: 'video-tf-graph-kf-count-zero.webm', mimeType: 'video/webm', buffer: webm })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible({ timeout: 15_000 })
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'video-tf-graph-kf-count-zero.webm')

  await page.getByRole('button', { name: 'トランスフォームキーフレーム', exact: true }).click()
  await expect(page.getByText('0件', { exact: true })).toBeVisible()
  await expect(page.getByText('再生ヘッド位置(クリップ内)に位置・スケール・回転・不透明度のキーフレームを追加できます')).toBeVisible()
  await expect(page.getByTestId('transform-kf-graph-editor')).toBeHidden()
})

test('インスペクター: 画像クリップのトランスフォームキーフレームのグラフエディターでキーフレーム0件時に件数表示が0件になる', async ({ page }) => {
  await goOnboarded(page)
  await page.setInputFiles('input[accept*="image"]', { name: 'image-tf-graph-kf-count-zero.png', mimeType: 'image/png', buffer: TINY_PNG })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible()
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'image-tf-graph-kf-count-zero.png')
...
test('インスペクター: テキストクリップのトランスフォームキーフレームのグラフエディターでキーフレーム0件時に件数表示が0件になる', async ({ page }) => {
  await goOnboarded(page)
  await addOpeningText(page)
  await clickTimelineClip(page, 'Opening')
...
test('インスペクター: 動画クリップのトランスフォームキーフレームのグラフエディターで最終キーフレーム削除後に件数表示が0件になる', async ({ page }) => {
  await goOnboarded(page)
  const webm = await makeTinyWebmVideo(page)
  await page.getByTitle('メディア').click()
  await page.setInputFiles('input[accept*="video"]', { name: 'video-tf-graph-kf-count-last-delete-zero.webm', mimeType: 'video/webm', buffer: webm })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible({ timeout: 15_000 })
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'video-tf-graph-kf-count-last-delete-zero.webm')
...
test('インスペクター: 画像クリップのトランスフォームキーフレームのグラフエディターで最終キーフレーム削除後に件数表示が0件になる', async ({ page }) => {
  await goOnboarded(page)
  await page.setInputFiles('input[accept*="image"]', { name: 'image-tf-graph-kf-count-last-delete-zero.png', mimeType: 'image/png', buffer: TINY_PNG })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible()
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'image-tf-graph-kf-count-last-delete-zero.png')
...
test('インスペクター: テキストクリップのトランスフォームキーフレームのグラフエディターで最終キーフレーム削除後に件数表示が0件になる', async ({ page }) => {
  await goOnboarded(page)
  await addOpeningText(page)
  await clickTimelineClip(page, 'Opening')
...
test('インスペクター: 動画クリップのトランスフォームキーフレームのグラフエディターで3点のキーフレームをすべて削除後に件数表示が0件になる', async ({ page }) => {
  await goOnboarded(page)
  const webm = await makeTinyWebmVideo(page)
  await page.getByTitle('メディア').click()
  await page.setInputFiles('input[accept*="video"]', { name: 'video-tf-graph-delete-3kf-count-zero.webm', mimeType: 'video/webm', buffer: webm })
  await expect(page.getByText('1件のメディアを追加しました')).toBeVisible({ timeout: 15_000 })
  await page.getByTitle('クリックで再生位置に追加').click()
  await clickTimelineClip(page, 'video-tf-graph-delete-3kf-count-zero.webm')
