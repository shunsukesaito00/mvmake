...
import {
  ...
  PROD_SMOKE_V2630_ADDITIONS,
  PROD_SMOKE_V2631_ADDITIONS,
  PROD_SMOKE_V2632_ADDITIONS,
  PROD_SMOKE_V2633_ADDITIONS,
  PROD_SMOKE_V2634_ADDITIONS,
  PROD_SMOKE_V2635_ADDITIONS,
  PROD_SMOKE_V2636_ADDITIONS,
  PROD_SMOKE_V2637_ADDITIONS,
  PROD_SMOKE_V2638_ADDITIONS,
  PROD_SMOKE_V2639_ADDITIONS,
  PROD_SMOKE_V2640_ADDITIONS,
  PROD_SMOKE_V2641_ADDITIONS,
  PROD_SMOKE_V2642_ADDITIONS,
  PROD_SMOKE_V2643_ADDITIONS,
  PROD_SMOKE_V2644_ADDITIONS,
  PROD_SMOKE_V2645_ADDITIONS,
  PROD_SMOKE_V2646_ADDITIONS,
  PROD_SMOKE_V2647_ADDITIONS,
  PROD_SMOKE_V2648_ADDITIONS,
  PROD_SMOKE_V2649_ADDITIONS,
  PROD_SMOKE_V2650_ADDITIONS,
  PROD_SMOKE_V2651_ADDITIONS,
  PROD_SMOKE_V2652_ADDITIONS,
  PROD_SMOKE_V2653_ADDITIONS,
  PROD_SMOKE_V2654_ADDITIONS,
  PROD_SMOKE_V2655_ADDITIONS,
  PROD_SMOKE_V2656_ADDITIONS,
  PROD_SMOKE_V2657_ADDITIONS,
  PROD_SMOKE_V2658_ADDITIONS,
  PROD_SMOKE_V2659_ADDITIONS,
  PROD_SMOKE_V2660_ADDITIONS,
  PROD_SMOKE_V2661_ADDITIONS,
  PROD_SMOKE_V2662_ADDITIONS,
  PROD_SMOKE_V2663_ADDITIONS,
  PROD_SMOKE_V2664_ADDITIONS,
  PROD_SMOKE_V300_ADDITIONS,
  PROD_SMOKE_V301_ADDITIONS,
  PROD_SMOKE_V302_ADDITIONS,
  PROD_SMOKE_V303_ADDITIONS,
  PROD_SMOKE_V304_ADDITIONS,
...
describe('prodSmokeAudit', () => {
  ...
  it('v2.6.30 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2630_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.31 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2631_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.32 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2632_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.33 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2633_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.34 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2634_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.35 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2635_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.36 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2636_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.37 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2637_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.38 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2638_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.39 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2639_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.40 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2640_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.41 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2641_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.42 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2642_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.43 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2643_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.44 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2644_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.45 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2645_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.46 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2646_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.47 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2647_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.48 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2648_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.49 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2649_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.50 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2650_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.51 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2651_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.52 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2652_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.53 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2653_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.54 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2654_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.55 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2655_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.56 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2656_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.57 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2657_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.58 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2658_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.59 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2659_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.60 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2660_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.61 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2661_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v2.6.62 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V2662_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })
  ...
  it('v3.0.0 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V300_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v3.0.1 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V301_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v3.0.2 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V302_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v3.0.3 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V303_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })

  it('v3.0.4 追加シナリオが basic.spec.ts に含まれる', () => {
    for (const label of PROD_SMOKE_V304_ADDITIONS) {
      expect(basic).toContain(label)
    }
  })
