...
describe('colorLooks', () => {
  ...
  it('colorAdjustmentsEqual が近似値を判定する', () => {
    expect(colorAdjustmentsEqual(
      { brightness: 0.08, contrast: 0.05, saturation: 0.12, hue: 0, temperature: 0, tint: 0 },
      { brightness: 0.0800001, contrast: 0.05, saturation: 0.12, hue: 0, temperature: 0, tint: 0 },
    )).toBe(true)
  })
  ...
  it('matchColorLookPreset が一致プリセット id を返す', () => {
    ...
    expect(matchColorLookPreset({ brightness: 0.3, contrast: 0, saturation: 0, hue: 0, temperature: 0, tint: 0 })).toBeNull()
  })
})
